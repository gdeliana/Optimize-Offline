...
- DormantOneDrive - The native fresh install startup behavour of Windows is to automatically install Onedrive. Setting DormantOneDrive to false makes no change to the native fresh install startup behavour. Setting DormantOneDrive to true stops, blocks and disables OneDrive from automatically installing during a fresh install or after a cumulative update or at any other point in time. Setting DormantOneDrive to true gives you complete control over when, if ever, you wwantg to isnsall, or not install OneDrive.
